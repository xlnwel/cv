...
def parse_cmd_args():
    ...
    parser.add_argument('--dataset', '-d', type=str, choices=['mnist', 'celeba'])
    parser.add_argument('--path', '-p', type=str, default='../../dataset')
...
class Dataset(object):
    ...
    Dataset
    """
    def __init__(self, batch_size, dataset_name, data_files):
        """
        Initalize the class
    ...
    def __call__(self):
        ...
        current_index = 0
        while current_index + self.batch_size <= self.shape[0]:
            data_batch = get_batch(
                self.data_files[current_index:current_index + self.batch_size],
                *self.shape[1:3],
                self.image_mode)
...
if __name__ == '__main__':
    ...
    download_extract(args.dataset, args.path)